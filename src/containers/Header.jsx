...
const Containers = {
	wave: Keyframes.Spring({
		...
		wave: async (next, cancel, ownProps) => {
			...
			await next({
				...
				marginLeft: 0,
				marginTop: 15,
				config: { ...config.gentle, duration: 150 },
			});
			await next({
				transform: 'rotate(0deg)',
				marginLeft: 19,
	...
	poke: Keyframes.Spring({
		...
		poke: async (next, cancel, ownProps) => {
			await next({
				marginLeft: 0,
				config: { easing: (t) => t, duration: 100 },
			});
			await next({
				marginLeft: 10,
				config: { easing: (t) => t, duration: 100 },
		...
		return (
			<div style={style.container}>
				...
				<div style={style.introduction} className="l">
					{"I'm "}
					<b>Liam Nields</b>, a full stack software engineer with
					an interest front end web developement.
					{' I '}
					love building useful and user friendly products.
				</div>
					...
					>
						{(emailStyle) => (
							<a
								...
								href="mailto:lnields3.0@gmail.com"
							>
								lnields3.0@gmail.com
							</a>
						)}
					</Spring>
				</div>
